...
eightgndhmrfouronexldvdvqnzxqjczfk1
        ...
        StringBuilder spelledNumber1 = new StringBuilder();

        for (String row : input.split("\n")) {
            for (int i = 0; i <= row.length(); i++) {
                if (Character.isDigit(row.charAt(i))) {
                    normalNumber.append(row.charAt(i));
                    break;
                }else{
                    spelledNumber1.append(row.charAt(i));
                }
                //check for spelled number in builder
                if(contrainsSpelledNumber(spelledNumber1.toString())!= 0){
                    normalNumber.append(Integer.toString(contrainsSpelledNumber(spelledNumber1.toString())));
                    break;
                }
            }
            spelledNumber1.setLength(0);
            //System.out.println(builder);
            for (int i = row.length(); i > 0; i--) {
                
                if (Character.isDigit(row.charAt(i-1))) {
                    normalNumber.append(row.charAt(i-1));
                    break;
                }else{
                    spelledNumber1.insert(0,row.charAt(i-1));
                }
                //check for spelled number in builder
                if(contrainsSpelledNumber(spelledNumber1.toString())!= 0){
                    normalNumber.append(Integer.toString(contrainsSpelledNumber(spelledNumber1.toString())));
            ...
            System.out.println(normalNumber.toString());
            finalValue += Integer.parseInt(normalNumber.toString());
            normalNumber.setLength(0);
            spelledNumber1.setLength(0);
        }
    ...
    public static int contrainsSpelledNumber(String s) {
        if (s.contains("one")) {
            return 1;
